...
import { ServiceCategory } from '../types/catalog-master';
import { X, Plus, Trash2, Edit3, Save, Layers } from 'lucide-react';
...
interface CategoryManagerModalProps {
  ...
  onSaveCategory: (cat: ServiceCategory) => void;
...
export function CategoryManagerModal({
  ...
  const handleEditClick = (cat: ServiceCategory) => {
    setEditingCat(cat);
    setName(cat.name);
    setSlug(cat.slug);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    toast.success(editingCat ? 'Kategori berhasil diperbarui!' : 'Kategori baru berhasil dibuat!');
    handleAddNew();
  };

  const handleDelete = (cat: ServiceCategory) => {
    if (cat.servicesCount && cat.servicesCount > 0) {
      toast.error(`Kategori "${cat.name}" tidak dapat dihapus karena masih menampung ${cat.servicesCount} layanan aktif.`);
      return;
    }
      ...
      <div className="relative w-full max-w-3xl bg-slate-50 border border-slate-200 rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden max-h-[85vh] z-10 animate-fade-in">
        ...
        <div className="w-full md:w-1/2 p-5 border-b md:border-b-0 md:border-r border-slate-200 flex flex-col justify-between overflow-y-auto">
          <div className="space-y-4">
            ...
            <div className="space-y-2 pr-1">
              {categories.length === 0 ? (
                <p className="text-[12px] text-slate-400 text-center py-8">Belum ada kategori yang dibuat.</p>
              ) : (
                categories
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400 font-mono">Slug: {cat.slug} | Order: {cat.sortOrder}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[10.5px] font-bold text-slate-500">
                          {cat.servicesCount || 0} layanan
                        </span>
                        <button
                          type="button"
                          onClick={(e) => {
        ...
        <div className="w-full md:w-1/2 p-5 bg-white flex flex-col justify-between">
          <div>
            ...
            <form onSubmit={handleSave} className="space-y-3.5 text-[12px] text-slate-600 font-semibold">
              <div>
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-600 mb-1">Custom Slug (Opsional)</label>
                <input
                  type="text"
                  placeholder="slug-otomatis-jika-kosong"
                  value={slug}
                  onChange={(e) => setSlug(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl outline-none focus:border-blue-400 bg-slate-50 focus:bg-white font-mono text-[11px]"
                />
              </div>
              ...
              <div className="flex items-center gap-2.5 pt-2 select-none">
                ...
                />
                <label htmlFor="cat-active" className="text-[12px] font-bold text-slate-700 cursor-pointer">
                  Aktifkan Kategori ini untuk Layanan Kasir
                </label>
              </div>
